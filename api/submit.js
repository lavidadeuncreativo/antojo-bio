...
module.exports=async function handler(req,res){
  ...
  const token=process.env.NOTION_TOKEN,dataSourceId=clean(process.env.NOTION_DATA_SOURCE_ID||process.env.NOTION_DATABASE_ID||DEFAULT_DATA_SOURCE_ID,100);if(!token)return sendJson(res,503,{ok:false,code:'NOTION_NOT_CONFIGURED',error:'Notion no está configurado.'});
  ...
  try{
    ...
    const response=await notionJson('/v1/pages',token,{method:'POST',body:JSON.stringify({parent:{data_source_id:dataSourceId},properties,markdown})});const data=await response.json().catch(()=>({}));if(!response.ok)return sendJson(res,502,{ok:false,error:'No fue posible registrar el pedido en Notion.',detail:data.message||null});return sendJson(res,201,{ok:true,folio,notionUrl:data.url,pageId:data.id,logoSaved:Boolean(logoUpload)});
  }catch(notionError){console.error('Registration failed',notionError);return sendJson(res,502,{ok:false,error:'No fue posible registrar el pedido.',detail:notionError.message||null})}
};